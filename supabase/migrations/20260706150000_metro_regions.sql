...
-- Data first: rows written under the blanket label are Greater Los Angeles
-- orgs (the original 11). Adding the new CHECK validates existing rows, so
-- this update must run before the constraint swap. (On a fresh `db reset`
-- the table is empty here and this is a no-op.)
